...
Given('a town where all seven days have wind speed {float} kmph, exactly at the surfing threshold', function (this: ActivityRankingWorld, value: number) {
  assert.equal(value, 25.0);
  this.selectedCoordinates = weatherLocations.boundaryWindExact;
});

Given('a town where all seven days have wind speed {float} kmph, just above the surfing threshold', function (this: ActivityRankingWorld, value: number) {
...
Given('a town where all seven days have rainfall {float} mmph, just below the indoor threshold', function (this: ActivityRankingWorld, value: number) {
  assert.equal(value, 4.9);
  this.selectedCoordinates = weatherLocations.boundaryRainLow;
});

Given('a town where all seven days have rainfall {float} mmph, exactly at the indoor threshold', function (this: ActivityRankingWorld, value: number) {
  assert.equal(value, 5.0);
  this.selectedCoordinates = weatherLocations.boundaryRainExact;
});

Given('a town where all seven days have rainfall {float} mmph, just above the indoor threshold', function (this: ActivityRankingWorld, value: number) {
